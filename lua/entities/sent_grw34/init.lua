-- SENTRY: GrW 34 Mortar | grenade | DMG:160 | Range:1800
AddCSLuaFile()
ENT.Type="anim" ENT.Base="base_anim" ENT.PrintName="GrW 34 Mortar Sentry"
...
function ENT:Initialize()
  ...
  self:SetHealth(500) self:SetMaxHealth(500)
  local p=self:GetPhysicsObject() if IsValid(p) then p:EnableMotion(false) end
  self.NextFireTime=0 self.FireDelay=3.5 self.BurstCount=0 self.BurstMax=1 self.BurstDelay=6.0
  self.NextBurstTime=0 self.Target=nil self.Range=1800 self.Damage=160 self.AmmoType="grenade"
...
function ENT:Fire()
  ...
  self:FireBullets({Num=1,Src=m,Dir=d,Spread=Vector(0.06,0.06,0),Tracer=1,Force=28,Damage=self.Damage,AmmoType=self.AmmoType,AttackerTable={self}})
