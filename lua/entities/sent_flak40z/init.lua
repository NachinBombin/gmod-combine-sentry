-- SENTRY: Flak 40 Zwilling | AR2AltFire | DMG:140 | Range:3200
AddCSLuaFile()
ENT.Type="anim" ENT.Base="base_anim" ENT.PrintName="Flak 40 Zwilling Sentry"
...
function ENT:Initialize()
  ...
  self:SetHealth(950) self:SetMaxHealth(950)
  local p=self:GetPhysicsObject() if IsValid(p) then p:EnableMotion(false) end
  self.NextFireTime=0 self.FireDelay=0.4 self.BurstCount=0 self.BurstMax=4 self.BurstDelay=2.5
  self.NextBurstTime=0 self.Target=nil self.Range=3200 self.Damage=140 self.AmmoType="AR2AltFire"
...
function ENT:Fire()
  ...
  self:FireBullets({Num=1,Src=m,Dir=d,Spread=Vector(0.02,0.02,0),Tracer=1,Force=22,Damage=self.Damage,AmmoType=self.AmmoType,AttackerTable={self}})
