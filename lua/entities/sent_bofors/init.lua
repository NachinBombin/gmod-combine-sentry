-- SENTRY: Bofors 40mm | AR2AltFire | DMG:100 | Range:3000
AddCSLuaFile()
ENT.Type="anim" ENT.Base="base_anim" ENT.PrintName="Bofors 40mm Sentry"
...
function ENT:Initialize()
  ...
  self:SetHealth(850) self:SetMaxHealth(850)
  local p=self:GetPhysicsObject() if IsValid(p) then p:EnableMotion(false) end
  self.NextFireTime=0 self.FireDelay=0.5 self.BurstCount=0 self.BurstMax=4 self.BurstDelay=2.0
  self.NextBurstTime=0 self.Target=nil self.Range=3000 self.Damage=100 self.AmmoType="AR2AltFire"
...
function ENT:Fire()
  ...
  self:FireBullets({Num=1,Src=m,Dir=d,Spread=Vector(0.02,0.02,0),Tracer=1,Force=18,Damage=self.Damage,AmmoType=self.AmmoType,AttackerTable={self}})
