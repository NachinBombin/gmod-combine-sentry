-- SENTRY: Bren LMG | smg1 | DMG:18 | Range:1600
AddCSLuaFile()
ENT.Type="anim" ENT.Base="base_anim" ENT.PrintName="Bren LMG Sentry"
...
function ENT:Initialize()
  ...
  self:SetHealth(400) self:SetMaxHealth(400)
  local p=self:GetPhysicsObject() if IsValid(p) then p:EnableMotion(false) end
  self.NextFireTime=0 self.FireDelay=0.09 self.BurstCount=0 self.BurstMax=8 self.BurstDelay=1.5
  self.NextBurstTime=0 self.Target=nil self.Range=1600 self.Damage=18 self.AmmoType="smg1"
...
function ENT:Fire()
  ...
  self:FireBullets({Num=1,Src=m,Dir=d,Spread=Vector(0.04,0.04,0),Tracer=1,Force=4,Damage=self.Damage,AmmoType=self.AmmoType,AttackerTable={self}})
  self:EmitSound("weapons/smg1/fire1.wav",85,math.random(95,105))
